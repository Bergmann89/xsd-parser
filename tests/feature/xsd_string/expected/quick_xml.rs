            ...
            loop {
                match &mut self.state {
                    FooTypeSerializerState::Init__ => {
                        self.state = FooTypeSerializerState::Text(
                    ...
                    FooTypeSerializerState::Text(x) => match x.next() {
...
pub mod quick_xml_deserialize {
    ...
    impl FooTypeDeserializer {
            ...
            R: xsd_parser::quick_xml::XmlReader,
        {
            ...
            Ok(Self {
                text: None,
                state: Box::new(FooTypeDeserializerState::Text(None)),
            })
        }
    }
    impl<'de> xsd_parser::quick_xml::Deserializer<'de, super::FooType> for FooTypeDeserializer {
            ...
            R: xsd_parser::quick_xml::XmlReader,
        {
            ...
            loop {
                event = match (
                    ...
                    (FooTypeDeserializerState::Text(Some(deserializer)), event) => {
                        ...
                        let event = match event {
                            ...
                            event => {
                                *self.state = FooTypeDeserializerState::Text(deserializer);
                        ...
                        if allow_any {
                            allow_any_fallback
                                .get_or_insert(FooTypeDeserializerState::Text(deserializer));
                        ...
                        *self.state = FooTypeDeserializerState::Text(None);
                        event
                    }
                    (FooTypeDeserializerState::Text(None), event) => match &event {
                            ...
                            match event {
                                Some(event @ (Event::Start(_) | Event::End(_))) => {
                                    *self.state = FooTypeDeserializerState::Done__;
                                    if allow_any {
                                        allow_any_fallback
                                            .get_or_insert(FooTypeDeserializerState::Text(None));
                                    }
                                    event
                                }
                                event @ (None | Some(_)) => {
            ...
            R: xsd_parser::quick_xml::XmlReader,
        {
            ...
            Ok(super::FooType {
                text: self
                    .text
                    .ok_or_else(|| ErrorKind::MissingElement("Text".into()))?,
            })
        }
    }
}
