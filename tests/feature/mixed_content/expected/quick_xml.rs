...
pub struct MixedSequenceType {
    pub text_before: Option<String>,
    pub fuu: i32,
    pub text_after_fuu: Option<String>,
    pub bar: String,
    pub text_after_bar: Option<String>,
}
...
pub mod quick_xml_deserialize {
    ...
    impl MixedAllTypeDeserializer {
            ...
            R: DeserializeReader,
        {
            let (Event::Start(x) | Event::Empty(x)) = &event else {
                *self.state = fallback
                    .take()
                    .unwrap_or(MixedAllTypeDeserializerState::Init__);
            ...
            if matches!(
                ...
                let output = <Mixed<i32> as WithDeserializer>::Deserializer::init(reader, event)?;
    ...
    enum MixedChoiceTypeDeserializerState {
        Init__,
        Next__,
        Content__(<super::MixedChoiceTypeContent as WithDeserializer>::Deserializer),
        Unknown__,
    }
    impl MixedChoiceTypeDeserializer {
        ...
        where
            R: DeserializeReader,
        {
            if let MixedChoiceTypeDeserializerState::Content__(deserializer) = state {
            ...
            R: DeserializeReader,
        {
            ...
            if artifact.is_none() {
                *self.state = fallback
                    .take()
                    .unwrap_or(MixedChoiceTypeDeserializerState::Next__);
            ...
            Ok(match artifact {
                ...
                DeserializerArtifact::Data(data) => {
                    ...
                    *self.state = MixedChoiceTypeDeserializerState::Next__;
                    ElementHandlerOutput::from_event(event, allow_any)
                }
                DeserializerArtifact::Deserializer(deserializer) => {
                    *self.state = MixedChoiceTypeDeserializerState::Content__(deserializer);
    ...
    impl MixedChoiceListTypeContentDeserializer {
        ...
        fn store_text(values: &mut Option<String>, value: String) -> Result<(), Error> {
            if values.is_some() {
    ...
    pub struct MixedSequenceTypeDeserializer {
        text_before: Option<String>,
        fuu: Option<i32>,
        text_after_fuu: Option<String>,
        bar: Option<String>,
        text_after_bar: Option<String>,
        state: Box<MixedSequenceTypeDeserializerState>,
    }
    #[derive(Debug)]
    enum MixedSequenceTypeDeserializerState {
        Init__,
        TextBefore(Option<<String as WithDeserializer>::Deserializer>),
        Fuu(Option<<i32 as WithDeserializer>::Deserializer>),
        TextAfterFuu(Option<<String as WithDeserializer>::Deserializer>),
        Bar(Option<<String as WithDeserializer>::Deserializer>),
        TextAfterBar(Option<<String as WithDeserializer>::Deserializer>),
        Done__,
        Unknown__,
    }
    impl MixedSequenceTypeDeserializer {
        ...
        fn store_text_before(&mut self, value: String) -> Result<(), Error> {
        ...
        fn store_text_after_fuu(&mut self, value: String) -> Result<(), Error> {
        ...
        fn store_text_after_bar(&mut self, value: String) -> Result<(), Error> {
            ...
            R: DeserializeReader,
        {
            ...
            let (event, allow_any) = loop {
                ...
                event = match (state, event) {
                    ...
                    (S::TextBefore(None), event @ (Event::Start(_) | Event::Empty(_))) => {
                        let output =
                            <String as WithDeserializer>::Deserializer::init(reader, event)?;
                        match self.handle_text_before(reader, output, &mut fallback)? {
                            ElementHandlerOutput::Continue { event, allow_any } => {
                    ...
                    (S::TextAfterFuu(None), event @ (Event::Start(_) | Event::Empty(_))) => {
                        let output =
                            <String as WithDeserializer>::Deserializer::init(reader, event)?;
                        match self.handle_text_after_fuu(reader, output, &mut fallback)? {
                            ElementHandlerOutput::Continue { event, allow_any } => {
                    ...
                    (S::TextAfterBar(None), event @ (Event::Start(_) | Event::Empty(_))) => {
                        let output =
                            <String as WithDeserializer>::Deserializer::init(reader, event)?;
                        match self.handle_text_after_bar(reader, output, &mut fallback)? {
                            ElementHandlerOutput::Continue { event, allow_any } => {
...
pub mod quick_xml_serialize {
    ...
    use xsd_parser::{
        quick_xml::{
            BytesEnd, BytesStart, BytesText, Error, Event, IterSerializer, WithSerializer,
        },
        xml::Mixed,
    };
    #[derive(Debug)]
            ...
            loop {
                match &mut *self.state {
                    MixedAllTypeSerializerState::Init__ => {
                        *self.state = MixedAllTypeSerializerState::TextBefore__;
                    ...
                    MixedAllTypeSerializerState::TextBefore__ => {
                        *self.state = MixedAllTypeSerializerState::Fuu(WithSerializer::serializer(
                            &self.value.fuu,
                            Some("tns:Fuu"),
                            false,
                        )?);
    ...
    pub(super) enum MixedChoiceTypeSerializerState<'ser> {
        Init__,
        TextBefore__,
        Content__(<super::MixedChoiceTypeContent as WithSerializer>::Serializer<'ser>),
        End__,
        Done__,
        Phantom__(&'ser ()),
    }
            ...
            loop {
                match &mut *self.state {
                    MixedChoiceTypeSerializerState::Init__ => {
                        *self.state = MixedChoiceTypeSerializerState::TextBefore__;
                        let mut bytes = BytesStart::new(self.name);
                        if self.is_root {
                            bytes.push_attribute((&b"xmlns:tns"[..], &super::NS_TNS[..]));
                        }
                    ...
                    MixedChoiceTypeSerializerState::TextBefore__ => {
                        *self.state = MixedChoiceTypeSerializerState::Content__(
                    ...
                    MixedChoiceTypeSerializerState::Content__(x) => match x.next().transpose()? {
                        Some(event) => return Ok(Some(event)),
