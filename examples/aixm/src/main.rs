...
use schema::message::AixmBasicMessage;

fn main() -> Result<(), Error> {
    let input_file = File::open("xml/COCESNA_20170516_sample.xml")
        .context("Unable to open example XML for reading")?;
    let reader = BufReader::new(input_file);
    let mut reader = IoReader::new(reader).with_error_info();
    let obj = AixmBasicMessage::deserialize(&mut reader)
        .context("Error while deserializing XML content")?;
