...
use xsd_parser::{
    ...
    Config, IdentType, Name, TypeIdent,
};
...
fn main() -> Result<(), Error> {
    ...
    let schema_file = cargo_dir
        .join("schema/aixm.aero/schema/5.2/5.2.0/message/AIXM_BasicMessage.xsd")
        .canonicalize()
    ...
    let config = Config::default()
        ...
        .with_quick_xml()
        .with_generate([(IdentType::Element, "message:AIXMBasicMessage")]);

    // Generate the code based on the configuration above.
    let modules = generate_modules(config).context("Error while generating code")?;
...
impl NamingTrait for CustomNaming {
    ...
    fn format_type_name(&self, s: &str) -> String {
        match s {
            "secondDefiningParameter" => "SecondDefiningParameterBase".to_string(),
            s => self.0.format_type_name(s),
        }
    }
    ...
    fn format_variant_name(&self, s: &str) -> String {
        match s {
            "+" => "Plus".to_string(),
            "-" => "Minus".to_string(),
            s if s.contains("UTC") => s
                .replace("UTC:", "Utc")
                .replace("UTC", "Utc")
                .replace(':', "_")
                .replace('+', "Plus")
                .replace('-', "Minus"),
            s => self.0.format_variant_name(s),
        }
    }
