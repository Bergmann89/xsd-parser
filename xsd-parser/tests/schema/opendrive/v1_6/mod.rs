...
fn replace_variant_names(mut types: MetaTypes) -> MetaTypes {
    for (ident, ty) in types.items.iter_mut() {
        if ident.name.as_named_str() == Some("t_road_signals_signalReference") {
            ty.display_name = Some("TRoadSignalsSignalReference2".into());
        }
...
fn generate_default() {
    generate_test(
        ...
        "tests/schema/opendrive/v1_6/expected/default.rs",
        config(),
    );
}
