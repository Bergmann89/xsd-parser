...
impl WithSerializer for Animal {
    ...
    fn serializer<'ser>(
        ...
        if (*self.0).as_any().is::<AnimalType>() {
            return self.0.serializer(Some("animal"), is_root);
        }
...
pub trait AnimalTypeTrait: Debug + AsAny + WithBoxedSerializer {}
...
impl WithSerializer for AnimalType {
    ...
    fn serializer<'ser>(
        ...
        if (*self.0).as_any().is::<DogType>() || (*self.0).as_any().is::<LabradorType>() {
            return Ok(Box::new(XsiTypeSerializer::new(
                self.0.serializer(None, is_root)?,
                name.unwrap_or("animal"),
                is_root,
            )));
        }
        self.0.serializer(None, is_root)
...
pub mod quick_xml_deserialize {
    ...
    pub enum AnimalDeserializer {
        Animal(<super::AnimalType as WithDeserializer>::Deserializer),
        ...
        fn init(
            ...
            if matches!(
                ...
                let DeserializerOutput {
                    ...
                    event,
                    allow_any,
                } = <super::AnimalType as WithDeserializer>::init(helper, event)?;
                return Ok(DeserializerOutput {
    ...
    #[derive(Debug)]
    pub enum AnimalTypeDeserializer {
    ...
    impl<'de> Deserializer<'de, super::AnimalType> for AnimalTypeDeserializer {
        fn init(
            ...
            if matches!(
                ...
                if matches!(
                    ...
                    return Ok(DeserializerOutput {
                        artifact: artifact
                            .map(|x| super::AnimalType(Box::new(x)), |x| Self::Animal(x)),
                        event,
                        allow_any,
                    });
                }
                if matches!(
                    ...
                    return Ok(DeserializerOutput {
                        artifact: artifact
                            .map(|x| super::AnimalType(Box::new(x)), |x| Self::Dog(x)),
                        event,
                        allow_any,
                    });
                }
                if matches!(
                    ...
                    return Ok(DeserializerOutput {
                        artifact: artifact
                            .map(|x| super::AnimalType(Box::new(x)), |x| Self::Labrador(x)),
                        event,
                        allow_any,
                    });
                }
        ...
        fn finish(self, helper: &mut DeserializeHelper) -> Result<super::AnimalType, Error> {
            match self {
                Self::Animal(x) => Ok(super::AnimalType(Box::new(x.finish(helper)?))),
                Self::Dog(x) => Ok(super::AnimalType(Box::new(x.finish(helper)?))),
                Self::Labrador(x) => Ok(super::AnimalType(Box::new(x.finish(helper)?))),
            }
        }
    }
    #[derive(Debug)]
