...
#[derive(Debug)]
pub struct HexType(pub HexBinary);
impl HexType {
    pub fn new(inner: HexBinary) -> Result<Self, ValidateError> {
        Self::validate_value(&inner)?;
        Ok(Self(inner))
    }
    #[must_use]
    pub fn into_inner(self) -> HexBinary {
        self.0
    }
    pub fn validate_value(value: &HexBinary) -> Result<(), ValidateError> {
...
impl From<HexType> for HexBinary {
    fn from(value: HexType) -> HexBinary {
        value.0
    }
}
impl TryFrom<HexBinary> for HexType {
    type Error = ValidateError;
    fn try_from(value: HexBinary) -> Result<Self, ValidateError> {
        Self::new(value)
    }
}
...
impl DeserializeBytes for HexType {
    fn deserialize_bytes(helper: &mut DeserializeHelper, bytes: &[u8]) -> Result<Self, Error> {
        let inner = HexBinary::deserialize_bytes(helper, bytes)?;
        Ok(Self::new(inner).map_err(|error| (bytes, error))?)
    }
}
