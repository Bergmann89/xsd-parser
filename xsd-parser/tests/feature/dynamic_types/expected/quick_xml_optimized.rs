...
pub mod quick_xml_deserialize {
    ...
    pub enum BaseDeserializerState {
        Init__,
        Intermediate(
            Option<super::IntermediateType>,
            Option<<super::IntermediateType as WithDeserializer>::Deserializer>,
            Option<<super::IntermediateType as WithDeserializer>::Deserializer>,
        ),
        Final(
            Option<super::FinalType>,
            Option<<super::FinalType as WithDeserializer>::Deserializer>,
            Option<<super::FinalType as WithDeserializer>::Deserializer>,
        ),
        Done__(super::Base),
        Unknown__,
    ...
    impl BaseDeserializer {
        fn find_suitable<'de>(
            ...
            if let Event::Start(x) | Event::Empty(x) = &event {
                ...
                if matches!(
                    ...
                    let output = <super::FinalType as WithDeserializer>::init(helper, event)?;
        ...
        fn store_intermediate(
            values: &mut Option<super::IntermediateType>,
            value: super::IntermediateType,
        ) -> Result<(), Error> {
            if values.is_some() {
        ...
        fn store_final_(
            values: &mut Option<super::FinalType>,
            value: super::FinalType,
        ) -> Result<(), Error> {
            if values.is_some() {
                Err(ErrorKind::DuplicateElement(RawByteStr::from_slice(
                    b"final",
        ...
        fn handle_intermediate<'de>(
            &mut self,
            helper: &mut DeserializeHelper,
            mut values: Option<super::IntermediateType>,
            fallback: Option<<super::IntermediateType as WithDeserializer>::Deserializer>,
            output: DeserializerOutput<'de, super::IntermediateType>,
        ) -> Result<ElementHandlerOutput<'de>, Error> {
            use BaseDeserializerState as S;
            let DeserializerOutput {
                artifact,
            ...
            match artifact {
                ...
                DeserializerArtifact::Data(data) => {
                    Self::store_intermediate(&mut values, data)?;
                    let data = Self::finish_state(helper, S::Intermediate(values, None, None))?;
                    *self.state__ = S::Done__(data);
                    Ok(ElementHandlerOutput::break_(event, allow_any))
                }
                DeserializerArtifact::Deserializer(deserializer) => {
                    *self.state__ = S::Intermediate(values, None, Some(deserializer));
                    Ok(ElementHandlerOutput::break_(event, allow_any))
                }
            }
        }
        fn handle_final_<'de>(
            &mut self,
            helper: &mut DeserializeHelper,
            mut values: Option<super::FinalType>,
            fallback: Option<<super::FinalType as WithDeserializer>::Deserializer>,
            output: DeserializerOutput<'de, super::FinalType>,
        ) -> Result<ElementHandlerOutput<'de>, Error> {
            use BaseDeserializerState as S;
            let DeserializerOutput {
                artifact,
            ...
            match artifact {
                ...
                DeserializerArtifact::Data(data) => {
                    Self::store_final_(&mut values, data)?;
                    let data = Self::finish_state(helper, S::Final(values, None, None))?;
                    *self.state__ = S::Done__(data);
                    Ok(ElementHandlerOutput::break_(event, allow_any))
                }
                DeserializerArtifact::Deserializer(deserializer) => {
                    *self.state__ = S::Final(values, None, Some(deserializer));
                    Ok(ElementHandlerOutput::break_(event, allow_any))
                }
            }
        }
...
pub mod quick_xml_serialize {
    ...
    pub(super) enum BaseSerializerState<'ser> {
        Init__,
        Intermediate(<super::IntermediateType as WithSerializer>::Serializer<'ser>),
        Final(<super::FinalType as WithSerializer>::Serializer<'ser>),
        Done__,
        Phantom__(&'ser ()),
    }
        ...
        fn next_event(
            ...
            loop {
                match &mut *self.state {
                    ...
                    BaseSerializerState::Intermediate(x) => match x.next(helper).transpose()? {
                        Some(event) => return Ok(Some(event)),
                        None => *self.state = BaseSerializerState::Done__,
                    },
                    BaseSerializerState::Final(x) => match x.next(helper).transpose()? {
                        Some(event) => return Ok(Some(event)),
                        None => *self.state = BaseSerializerState::Done__,
                    },
    ...
    impl<'ser> Serializer<'ser> for BaseSerializer<'ser> {
        fn next(&mut self, helper: &mut SerializeHelper) -> Option<Result<Event<'ser>, Error>> {
            match self.next_event(helper) {
                Ok(Some(event)) => Some(Ok(event)),
                Ok(None) => None,
