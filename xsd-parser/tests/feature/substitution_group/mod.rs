...
fn generate_default() {
    generate_test(
        "tests/feature/substitution_group/schema.xsd",
        "tests/feature/substitution_group/expected/default.rs",
        config(),
    );
}
...
fn generate_default_optimized() {
    generate_test(
        "tests/feature/substitution_group/schema.xsd",
        "tests/feature/substitution_group/expected/default_optimized.rs",
...
fn generate_quick_xml() {
    generate_test(
        "tests/feature/substitution_group/schema.xsd",
        "tests/feature/substitution_group/expected/quick_xml.rs",
...
#[cfg(not(feature = "update-expectations"))]
fn read_quick_xml() {
    use quick_xml::{AnimalType, DogType, LabradorType, List};

    let obj = crate::utils::quick_xml_read_test::<List, _>(
        "tests/feature/substitution_group/example/default.xml",
    );

    let mut items = obj.animal.iter();

    // The head element `animal` may appear directly.
    let item = items.next().unwrap();
    let item = item.0.as_any().downcast_ref::<AnimalType>().unwrap();
    assert_eq!(item.id, 1);

    // `<dog>` substitutes `<animal>` and is selected by its element name.
    let item = items.next().unwrap();
    let item = item.0.as_any().downcast_ref::<DogType>().unwrap();
    assert_eq!(item.id, 2);
    assert_eq!(item.name, "Rex");

    // `<labrador>` (transitively) substitutes `<animal>` via `<dog>`.
    let item = items.next().unwrap();
...
fn write_quick_xml() {
    ...
    let obj = List {
        animal: vec![
            Animal(Box::new(AnimalType { id: 1 })),
            Animal(Box::new(DogType {
                id: 2,
                name: "Rex".into(),
            })),
            Animal(Box::new(LabradorType {
                id: 3,
                name: "Buddy".into(),
                color: "black".into(),
            })),
...
fn generate_quick_xml_optimized() {
    generate_test(
        "tests/feature/substitution_group/schema.xsd",
        "tests/feature/substitution_group/expected/quick_xml_optimized.rs",
        config()
            .with_quick_xml()
