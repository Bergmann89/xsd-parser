...
use xsd_parser_types::{
    ...
    quick_xml::{
        BoxedSerializer, Error, WithBoxedSerializer, WithDeserializer, WithSerializer,
        XsiTypeSerializer,
    },
    AsAny,
};
...
impl WithSerializer for Animal {
    ...
    fn serializer<'ser>(
        ...
        if (*self.0).as_any().is::<CatType>() {
            return self.0.serializer(Some("cat"), is_root);
        }
...
pub trait DogTypeTrait: Debug + AsAny + WithBoxedSerializer {}
...
impl WithSerializer for DogType {
    ...
    fn serializer<'ser>(
        ...
        if (*self.0).as_any().is::<LabradorType>() || (*self.0).as_any().is::<BeagleType>() {
            return Ok(Box::new(XsiTypeSerializer::new(
                self.0.serializer(None, is_root)?,
                name.unwrap_or("dog"),
                is_root,
            )));
...
pub mod quick_xml_deserialize {
    ...
    pub enum AnimalDeserializer {
        Cat(<super::CatType as WithDeserializer>::Deserializer),
    ...
    impl<'de> Deserializer<'de, super::Animal> for AnimalDeserializer {
        fn init(
            ...
            if matches!(
                ...
                let DeserializerOutput {
                    ...
                    event,
                    allow_any,
                } = <super::CatType as WithDeserializer>::init(helper, event)?;
                return Ok(DeserializerOutput {
                    artifact: artifact.map(|x| super::Animal(Box::new(x)), |x| Self::Cat(x)),
                    event,
                    allow_any,
                });
            }
        ...
        fn finish(self, helper: &mut DeserializeHelper) -> Result<super::Animal, Error> {
            match self {
                Self::Cat(x) => Ok(super::Animal(Box::new(x.finish(helper)?))),
    ...
    #[derive(Debug)]
    pub enum DogTypeDeserializer {
        Labrador(<super::LabradorType as WithDeserializer>::Deserializer),
        Beagle(<super::BeagleType as WithDeserializer>::Deserializer),
    }
    impl<'de> Deserializer<'de, super::DogType> for DogTypeDeserializer {
            ...
            if matches!(
                ...
                if let Some(type_name) = helper.get_dynamic_type_from_attrib(&event)? {
                    ...
                    if matches!(
                        ...
                        return Ok(DeserializerOutput {
                            artifact: artifact
                                .map(|x| super::DogType(Box::new(x)), |x| Self::Labrador(x)),
                            event,
                            allow_any,
                        });
                    }
                    if matches!(
                        ...
                        return Ok(DeserializerOutput {
                            artifact: artifact
                                .map(|x| super::DogType(Box::new(x)), |x| Self::Beagle(x)),
                            event,
                            allow_any,
                        });
                    }
        ...
        fn finish(self, helper: &mut DeserializeHelper) -> Result<super::DogType, Error> {
            match self {
                Self::Labrador(x) => Ok(super::DogType(Box::new(x.finish(helper)?))),
                Self::Beagle(x) => Ok(super::DogType(Box::new(x.finish(helper)?))),
            }
        }
    }
    #[derive(Debug)]
