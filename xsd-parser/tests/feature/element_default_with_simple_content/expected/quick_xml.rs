...
impl WithSerializer for FooBarType {
    type Serializer<'x> = quick_xml_serialize::FooBarTypeSerializer<'x>;
    fn serializer<'ser>(
        &'ser self,
        name: Option<&'ser str>,
        is_root: bool,
    ) -> Result<Self::Serializer<'ser>, Error> {
        Ok(quick_xml_serialize::FooBarTypeSerializer {
            value: self,
            state: Box::new(quick_xml_serialize::FooBarTypeSerializerState::Init__),
            name: name.unwrap_or("FooBar"),
            is_root,
        })
    }
}
impl WithDeserializer for FooBarType {
    type Deserializer = quick_xml_deserialize::FooBarTypeDeserializer;
}
pub mod quick_xml_deserialize {
    ...
    pub struct FooTypeDeserializer {
        bar: Option<super::FooBarType>,
        state__: Box<FooTypeDeserializerState>,
    }
    #[derive(Debug)]
    enum FooTypeDeserializerState {
        Init__,
        Bar(Option<<super::FooBarType as WithDeserializer>::Deserializer>),
        Done__,
        Unknown__,
    }
    impl FooTypeDeserializer {
        ...
        fn store_bar(&mut self, value: super::FooBarType) -> Result<(), Error> {
            if self.bar.is_some() {
        ...
        fn handle_bar<'de>(
            &mut self,
            helper: &mut DeserializeHelper,
            output: DeserializerOutput<'de, super::FooBarType>,
    ...
    impl<'de> Deserializer<'de, super::FooType> for FooTypeDeserializer {
        ...
        fn finish(mut self, helper: &mut DeserializeHelper) -> Result<super::FooType, Error> {
            ...
            Ok(super::FooType { bar: self.bar })
        }
    }
    #[derive(Debug)]
    pub struct FooBarTypeDeserializer {
        baz: String,
        content: Option<bool>,
        state__: Box<FooBarTypeDeserializerState>,
    }
    #[derive(Debug)]
    ...
    impl FooBarTypeDeserializer {
        fn from_bytes_start(
            ...
            Ok(Self {
                baz: baz.ok_or_else(|| ErrorKind::MissingAttribute("Baz".into()))?,
                content: None,
                state__: Box::new(FooBarTypeDeserializerState::Init__),
            })
        }
        fn finish_state(
            &mut self,
            helper: &mut DeserializeHelper,
            state: FooBarTypeDeserializerState,
        ) -> Result<(), Error> {
        ...
        fn store_content(&mut self, value: bool) -> Result<(), Error> {
        ...
        ) -> DeserializerResult<'de, super::FooBarType> {
            use FooBarTypeDeserializerState as S;
            let DeserializerOutput {
                artifact,
                event,
                allow_any,
    ...
    impl<'de> Deserializer<'de, super::FooBarType> for FooBarTypeDeserializer {
        fn init(
            helper: &mut DeserializeHelper,
            event: Event<'de>,
        ) -> DeserializerResult<'de, super::FooBarType> {
        ...
        fn finish(mut self, helper: &mut DeserializeHelper) -> Result<super::FooBarType, Error> {
            let state = replace(&mut *self.state__, FooBarTypeDeserializerState::Unknown__);
            self.finish_state(helper, state)?;
            Ok(super::FooBarType {
                baz: self.baz,
...
pub mod quick_xml_serialize {
    ...
    pub(super) enum FooTypeSerializerState<'ser> {
        Init__,
        Bar(IterSerializer<'ser, Option<&'ser super::FooBarType>, super::FooBarType>),
        End__,
        Done__,
        Phantom__(&'ser ()),
    }
            ...
            loop {
                match &mut *self.state {
                    FooTypeSerializerState::Init__ => {
                        *self.state = FooTypeSerializerState::Bar(IterSerializer::new(
                    ...
                    FooTypeSerializerState::Bar(x) => match x.next(helper).transpose()? {
    ...
    #[derive(Debug)]
    pub struct FooBarTypeSerializer<'ser> {
        pub(super) value: &'ser super::FooBarType,
        pub(super) state: Box<FooBarTypeSerializerState<'ser>>,
        pub(super) name: &'ser str,
        pub(super) is_root: bool,
    }
    #[derive(Debug)]
    pub(super) enum FooBarTypeSerializerState<'ser> {
        Init__,
        Content__(<bool as WithSerializer>::Serializer<'ser>),
        End__,
        Done__,
        Phantom__(&'ser ()),
    }
    impl<'ser> FooBarTypeSerializer<'ser> {
        fn next_event(
            &mut self,
            helper: &mut SerializeHelper,
        ) -> Result<Option<Event<'ser>>, Error> {
            loop {
                match &mut *self.state {
                    FooBarTypeSerializerState::Init__ => {
                        *self.state = FooBarTypeSerializerState::Content__(
                    ...
                    FooBarTypeSerializerState::Content__(x) => match x.next(helper).transpose()? {
                        Some(event) => return Ok(Some(event)),
                        None => *self.state = FooBarTypeSerializerState::End__,
                    },
                    FooBarTypeSerializerState::End__ => {
                        *self.state = FooBarTypeSerializerState::Done__;
                        return Ok(Some(Event::End(BytesEnd::new(self.name))));
                    }
                    FooBarTypeSerializerState::Done__ => return Ok(None),
                    FooBarTypeSerializerState::Phantom__(_) => unreachable!(),
                }
            }
        }
    }
    impl<'ser> Serializer<'ser> for FooBarTypeSerializer<'ser> {
        fn next(&mut self, helper: &mut SerializeHelper) -> Option<Result<Event<'ser>, Error>> {
            match self.next_event(helper) {
                Ok(Some(event)) => Some(Ok(event)),
                Ok(None) => None,
